...
//
// Arrays have powerful built in methods that replace many of the loops you
// would otherwise write by hand. The three most important to start with are
// map, filter, and find. Each one takes a function and runs it on every item
// in the array.
//
// Before this lesson you looped through arrays manually. These methods do
// the looping for you and make your code shorter and clearer.

// ---------------------------------------------------------------------
// A Quick Reminder About Functions in Arguments
// ---------------------------------------------------------------------
// These methods take a function as their input. You usually write that
// function as an arrow function right inside the method call.
...
// Here forEach runs the arrow function once for each item, passing the item
// in as number. Keep this pattern in mind because map, filter, and find all
// work the same way.

// ---------------------------------------------------------------------
// forEach
// ---------------------------------------------------------------------
// forEach runs a function once for every item in the array. It does not
// return anything. You use it when you just want to do something with each
// item, like printing.
...
// forEach is like a cleaner for of loop. But it cannot build a new array or
// stop early. For that you need the other methods.

// ---------------------------------------------------------------------
// map
// ---------------------------------------------------------------------
// map creates a new array by transforming every item. Whatever the function
// returns becomes the new item. The new array is always the same length as
// the original.
...
// Each item goes into the function, and the returned value takes its place
// in the new array.
...
// The key idea. map means take every item and change it into something new,
// keeping the same number of items.

// ---------------------------------------------------------------------
// filter
// ---------------------------------------------------------------------
// filter creates a new array containing only the items that pass a test.
// The function you give it must return true or false. Items that return
// true are kept. Items that return false are removed.
...
// The key idea. filter means keep only the items that pass the test. The
// new array may be smaller than the original.

// ---------------------------------------------------------------------
// find
// ---------------------------------------------------------------------
// find returns the first item that passes a test. Unlike filter, it returns
// a single item, not an array. If nothing matches, it returns undefined.
...
// The key idea. find gives you the first matching item, or undefined if
// none match. Use it when you want one specific item.
...
//
// A simple way to remember. map changes items. filter removes items. find
// locates one item.
...
// Each method works on the result of the one before it.
...
// First filter keeps the even numbers. Then map multiplies each by 10.
// Reading a chain from top to bottom tells the whole story.

// ---------------------------------------------------------------------
// Comparing to a Manual Loop
// ---------------------------------------------------------------------
// Here is the same task done both ways so you can see how much shorter
// these methods are.
//
// With a manual loop.
...
for (const price of prices) {
    if (price > 1000) {
        expensive2.push(price)
    }
}
console.log(expensive2)   // [1200, 1500]

// With filter.

const expensive3 = prices.filter((price) => price > 1000)
console.log(expensive3)   // [1200, 1500]
