...
//
// This lesson covers more array methods. reduce combines all items into a
// single value. sort arranges items in order. some and every check the
// whole array against a condition.

// ---------------------------------------------------------------------
// reduce
// ---------------------------------------------------------------------
// reduce takes all the items in an array and combines them into one single
// value. It is the method beginners find hardest, so we go slowly.
//
...
//
// The function takes two things: a running total called sum here, and the
// current item called number.
//
...
//
// You can name the two parts anything. A common style uses acc, short for
// accumulator, meaning the value that builds up.

const prices = [500, 1200, 800]

const total2 = prices.reduce((acc, price) => acc + price, 0)

console.log(total2)   // 2500

// Finding the largest value with reduce.

const numbers2 = [5, 82, 91, 67, 78]

const highest = numbers2.reduce((max, number) => {
    if (number > max) {
        return number
    }
    return max
...
// The key idea. reduce turns a whole array into one value, by carrying a
// running result through every item.

// ---------------------------------------------------------------------
// sort
// ---------------------------------------------------------------------
// sort arranges the items of an array in order. Important: sort changes
// the original array, it does not make a copy.
...
//
// This surprises people. By default sort treats items as text, which gives
// wrong results for numbers.

const numbers3 = [10, 5, 40, 25, 100]

numbers3.sort()
console.log(numbers3)   // [10, 100, 25, 40, 5]  wrong!

// To sort numbers correctly, give sort a compare function.

const numbers4 = [10, 5, 40, 25, 100]

numbers4.sort((a, b) => a - b)   // ascending, small to big
console.log(numbers4)   // [5, 10, 25, 40, 100]

numbers4.sort((a, b) => b - a)   // descending, big to small
console.log(numbers4)   // [100, 40, 25, 10, 5]

// The rule to remember. For numbers small to big, use a minus b. For big to
// small, use b minus a.

// Protecting the original
//
// Since sort changes the original, make a copy first if you want to keep
// the original order. Use the spread operator.

const numbers5 = [10, 5, 40, 25]

const sorted = [...numbers5].sort((a, b) => a - b)

console.log(sorted)    // [5, 10, 25, 40]
console.log(numbers5)   // [10, 5, 40, 25], original safe
...
const hasEven = numbers7.some((number) => number % 2 === 0)
console.log(hasEven)   // true, at least one even number exists

const hasBig = numbers7.some((number) => number > 100)
...
// every
// ---------------------------------------------------------------------
// every checks if all items pass a test. It returns true only if every
// single item matches.

const numbers8 = [2, 4, 6, 8]

const allEven = numbers8.every((number) => number % 2 === 0)
console.log(allEven)   // true, they are all even

const allBig = numbers8.every((number) => number > 5)
...
// Combining Methods
// ---------------------------------------------------------------------
// These methods combine beautifully with map and filter from the previous
// lesson.
//
// Total of all prices above 1000.

const prices2 = [500, 1200, 800, 1500, 300]

const total3 = prices2
    .filter((price) => price > 1000)   // [1200, 1500]
    .reduce((sum, price) => sum + price, 0)

console.log(total3)   // 2700
...
// reduce combines into one value. sort orders items but changes the
// original. some checks if any match. every checks if all match.
